...
ALTER TABLE FF_POTENTIAL_FUNDS
ADD PERCENTAGE_FEE NUMBER(12, 10);
...
UPDATE FF_POTENTIAL_FUNDS PF
SET PERCENTAGE_FEE = (
    SELECT SUM(AC.PERCENTAGE_FEE * FA.PERCENT_OF_FUND) / 100
    FROM FF_ASSET_CLASSES AC
    JOIN FF_FUND_ASSETS FA 
        ON AC.ASSET_CLASS_ID = FA.ASSET_CLASS_ID
    WHERE FA.FUND_ID = PF.FUND_ID
);
